...
class NoteArray extends Component{
    ...
    render(){
        const array = this.props.noteArray.filter(note => note.show).map(note => <Note text={note.text}
                                                             key={note.id}
                                                             id={note.id}
                                                             done={note.done} />)
        return <div className='noteArray'>
            {!array[0] && <h1 className="noteArray--h1" style={{color: "#d3d3d3"}}>Add a note...</h1>}
            {array}
        </div>
    }
}
